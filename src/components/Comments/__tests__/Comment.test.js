...
describe("components/Comments/Comment", () => {
    ...
    beforeEach(() => {
        ...
        CommitForm.mockImplementation(({ onSubmit }) => (
            <div
                data-testid="commit-form"
                onClick={() => onSubmit({ message: "edited message" })}
            ></div>
        ));
    });
    it("renders the comment", async () => {
        render(<Comment {...mockProps} />);
        expect(screen.getByTestId("comment-message")).toHaveTextContent(
            "message",
        );
    });
    it("allows the comment creator to edit the comment", async () => {
        ...
        expect(mockProps.onEdit).toHaveBeenCalledWith("abc-comment-id", {
    ...
    it("does not show the dropdown if the user did not create the comment", async () => {
        ...
        render(<Comment {...mockProps} />);
        expect(
            screen.queryByTestId("comment-dropdown"),
