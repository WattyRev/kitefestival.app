...
describe('Alert', () => {
    it('allows a component to trigger a success alert', async () => {
        const MockConsumer = () => {
            ...
            return <button data-testid="open-alert" onClick={() => openAlert('boogers', 'success')}>Open Alert</button>
        }
        render(
            <AlertProvider>
                <MockConsumer />
        ...
        expect(screen.getByTestId('alert-message')).toHaveTextContent('boogers');
