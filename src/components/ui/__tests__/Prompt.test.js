...
describe('Prompt', () => {
    it('allows a component to trigger a prompt', async () => {
        const MockConsumer = () => {
            ...
            return <button onClick={() => openPrompt('boogers')}>Open Prompt</button>
        }
        render(
            <PromptProvider>
                <MockConsumer />
        ...
        expect(screen.getByTestId('prompt-message')).toHaveTextContent('boogers');
    });
        ...
        it('uses the provided promptType as the input type', async () => {
            const MockConsumer = () => {
                ...
                function moveToNextType() {
                    const nextType = TEXT_INPUT_TYPES[TEXT_INPUT_TYPES.indexOf(inputType) + 1] || null;
                    setInputType(nextType);
                }
                return (
                    <>
                        <button onClick={moveToNextType} data-testid="next">Move to next type</button>
                        <div data-testid="type">{inputType}</div>
                        <button onClick={() => openPrompt('boogers', inputType).catch(() => {})} data-testid="open">Open Prompt</button>
                    </>
                );
            }
            render(
            ...
            for (const type of TEXT_INPUT_TYPES) {
                await waitFor(() => expect(screen.getByTestId('type')).toHaveTextContent(type));
                await userEvent.click(screen.getByTestId('open'));
                expect(screen.getByTestId('prompt-input')).toHaveAttribute('type', type);
                await userEvent.click(screen.getByTestId('prompt-cancel'));
