...
describe('CreateActivityForm', () => {
    ...
    it('allows an editor to create an activity', async () => {
        ...
        expect(onSubmit).toHaveBeenCalledWith({ title: 'Cool Activity', description: 'This is a cool activity' });
    });
    it('disables the save button while pending', async () => {
        ...
        resolveSubmit();
        await waitFor(() => expect(screen.getByTestId('save-activity')).not.toHaveAttribute('disabled'));
    });
