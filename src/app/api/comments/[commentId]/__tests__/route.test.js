...
describe('/api/comments/commentId/route', () => {
    ...
    describe('DELETE', () => {
        it('allows an editor to delete a comment', async () => {
            ...
            const response = await DELETE(null, { params: { commentId: 'abc' } });

            expect(sql).toHaveBeenCalledWith(['DELETE FROM comments WHERE id = ', ''], 'abc');
        ...
        it('returns a 403 if the user is not the comment author', async () => {
            ...
            expect(response).toEqual({ data: { message: 'You are not authorized to delete this comment' }, status: 403 });
        });
    });
    describe('PATCH', () => {
        it('allows a comment creator to edit a comment', async () => {
            ...
            expect(sql).toHaveBeenCalledWith(['UPDATE comments SET message = ', ', edited = true, userName = ', ' WHERE id = ', ''], 'updated message', 'user name', 'abc-comment-id');
        ...
        it('returns a 400 if there is no userName cookie', async () => {
            ...
            expect(sql).not.toHaveBeenCalledWith(['UPDATE comments SET message = ', ', edited = true, userName = ', ' WHERE id = ', ''], 'updated message', 'user name', 'abc-comment-id');
            expect(response).toEqual({ data: { message: 'No user name set' }, status: 400 });
        });
        it('returns a 400 is no message is provided', async () => {
            ...
            expect(sql).not.toHaveBeenCalledWith(['UPDATE comments SET message = ', ', edited = true, userName = ', ' WHERE id = ', ''], 'updated message', 'user name', 'abc-comment-id');
            expect(response).toEqual({ data: { message: 'No message provided' }, status: 400 });
        });
        it('returns a 401 if no passcode is provided', async () => {
            cookieValues.passcode = null;
            validatePasscode.mockRejectedValue(new NoPasscodeError());
