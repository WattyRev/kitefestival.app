    ...
    describe('POST', () => {
        ...
        it('should create a new comment', async () => {
            ...
            expect(sql).toHaveBeenCalledWith([
                'INSERT INTO comments (id, message, activityid, userid, username, createtime) VALUES (', ', ', ', ', ', ', ', ', ', now())'],
                'uuid',
                'booger message',
                '2',
                'user id',
        ...
        it('should return a 400 if no userId is set', async () => {
            ...
            const response = await POST(mockReq);
            expect(response).toEqual({ data: { message: 'No user ID or name set' }, status: 400 });
        });
        ...
        it('should return a 400 if no activityId is set', async () => {
            ...
            const response = await POST(mockReq);
            expect(response).toEqual({ data: { message: 'No activityId provided' }, status: 400 });
        });
    });
});