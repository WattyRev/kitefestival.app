...
describe("events/[eventId]/activate", () => {
  ...
  it("activates the event and deactivates others", async () => {
    sql
      .mockResolvedValueOnce({ rows: [{ id: "e1", name: "X" }] }) // exists
      .mockResolvedValueOnce({ rows: [] }) // deactivate all
      .mockResolvedValueOnce({ rows: [{ id: "e1", name: "X", start_date: null, end_date: null, description: null, location: null, is_active: true, created_at: "", updated_at: "" }] }); // activate

    const res = await POST(null, { params: { eventId: "e1" } });
    const data = await res.json();
    expect(data.activeEvent).toMatchObject({ id: "e1", name: "X", isActive: true });
