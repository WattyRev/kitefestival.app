...
describe('events', () => {
    describe('getEvents', () => {
        ...
        it('retrieves events with all valid columns', async () => {
            ...
            expect(sql.query).toHaveBeenCalledWith('SELECT id, name, slug, description FROM events');
        ...
        it('retrieves events with a subset of valid columns', async () => {
            ...
            expect(sql.query).toHaveBeenCalledWith('SELECT id, name FROM events');
        ...
        it('throws an error when no valid columns are requested', async () => {
            await expect(getEvents({ columns: ['invalidColumn'] })).rejects.toThrow('No valid columns requested');
        });
    });
});