...
jest.mock("../../../../components/HomePage", () => ({ __esModule: true, default: ({ activities, musicLibrary }) => (
  <div>
...
describe("EventPage", () => {
  it("maps rows to props and parses JSON music", async () => {
    sql
      .mockResolvedValueOnce({ rows: [
        { id: "a1", title: "T", description: "D", sortindex: 0, scheduleindex: null, music: "[\"s1\",\"s2\"]" }
      ] })
  ...
  it("falls back to array when music is not JSON", async () => {
    sql
      .mockResolvedValueOnce({ rows: [
        { id: "a1", title: "T", description: "D", sortindex: 0, scheduleindex: null, music: "Not JSON" }
      ] })
      .mockResolvedValueOnce({ rows: [] });
